...
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
  if (msg.from === POPUP_SCREEN && msg.subject === HANDLE_LOAD_EXTENSION) {
    ...
    if (accessToken) {
      accessTokenObject = { access_token, customer_id } =
        JSON.parse(accessToken);
      if (
        msg.currentURL.includes(
          "https://beta.tala.xyz/bao-hiem-so/thong-tin-hop-dong"
        )
      ) {
        const policyID = getElementByXpath(
          '//*[@id="__next"]/div[1]/main/div[2]/div[2]/div[3]'
        ).textContent.split("-")[2];
