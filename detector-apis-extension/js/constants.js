...
// Lowered from 500 (1.1.4) back to 200. This is not just a storage-size knob:
// REQUEST_ORDER_KEY holds the whole id list in ONE chrome.storage.local value
// that trackAndEvict rewrites on EVERY request, so the cap directly sets how
// many bytes the hottest write in the extension moves each time. At 500 that
// write was 2.5x larger than it had ever been, on a LevelDB database that a
// real user reported corrupting ("Corruption: block checksum mismatch", see
// safeStorageSet in js/background.js). Anyone who wants more can still raise
// it to 1000 on the options page.
