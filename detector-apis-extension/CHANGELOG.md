...
- **Automatic recovery attempt for the same bug**, so users no longer have to notice an empty popup and manually uninstall+reinstall. Two synthetic-probe designs were tried and rejected first: a canary fetch from a content script turns out to be subject to the host page's own CSP `connect-src` (would false-positive as "broken" on any CSP-strict site regardless of the actual bug), and a canary fetch from the service worker itself is invisible to `chrome.webRequest` by design (Chrome doesn't route an extension's own outgoing requests through webRequest at all). Landed on comparing two timestamps built from real traffic instead: `js/background.js` now stamps `LAST_PAGE_TRAFFIC_KEY` whenever it receives a `DETECTOR_APIS_RESPONSE_BODY` message from any tab (proof a page completed a real fetch/XHR and the content-script→background messaging channel, unaffected by page CSP, is alive) and `LAST_WEBREQUEST_SEEN_KEY` whenever its trackable `onBeforeRequest`/`onHeadersReceived` listeners actually fire (both ends, not just request-start, so one slow in-flight request doesn't read as a growing gap). Since every response has an earlier request, real traffic advancing the first timestamp while the second falls more than 2 minutes behind is direct evidence `chrome.webRequest` itself has stopped being routed events — at which point it calls `chrome.runtime.reload()` (same soft-restart as the "Reload" button in `chrome://extensions`; unlike uninstall+reinstall this does not clear `chrome.storage.local`, so no captured history or Options-page config is lost), capped at 3 attempts (resetting once a check finds the gap closed) so a page whose traffic is legitimately invisible to `chrome.webRequest` — e.g. a Service Worker answering `fetch()` from Cache Storage, which never touches the network layer webRequest observes — doesn't get reloaded every 2 minutes forever for no reason. Whether `chrome.runtime.reload()` actually clears the underlying Chromium-side routing bug is **not confirmed** — this is a best-effort auto-recovery attempt layered on top of the existing heartbeat, not a proven fix for the root cause. **Confirmed by live testing that `chrome.runtime.reload()` alone does not reliably fix this** — see below.
- **Two more recovery attempts, since `reload()` alone was confirmed not to be enough.** Each retry now also calls `chrome.runtime.requestUpdateCheck()` first — a distinct mechanism (asks Chrome to check for/apply a real update immediately) that might reset what a soft `reload()` doesn't, though this is equally unconfirmed and likely a no-op once already on the latest version. More importantly: once all `MAX_AUTO_RELOAD_ATTEMPTS` are exhausted without the gap closing, the toolbar now switches to a visible red "!" badge and a tooltip explaining the extension needs a manual reinstall, and clicking the icon opens `chrome://extensions` directly instead of an empty, broken capture popup (`chrome.action.setPopup("")` while broken so the click isn't swallowed by the normal popup). This doesn't make reinstalling automatic — nothing in the extension API can — but a user silently getting an extension that looks fine while capturing nothing is worse than one that visibly tells them it's broken.

### Changed
- **Default max tracked requests raised from 150 to 500.** Applies to anyone who has never explicitly set a value on the Options page (not just fresh installs — this is most existing users, since the setting was previously easy to never touch), tripling their tracked-request storage/read volume with the update. Anyone who already saved a custom value on the Options page keeps it untouched; the page's own configurable range (20-1000) is unchanged.
- **Requests from a closed tab are now cleared immediately** (`chrome.tabs.onRemoved`), instead of only disappearing once normal eviction happened to reach them — previously they could sit in storage and clutter "All tabs" view indefinitely.
- **3xx redirects get their own badge color** (blue, matching the status filter's existing "3xx" bucket) instead of being shown in the same red as 4xx/5xx errors and network failures.

### Fixed
- **XHR responses with `responseType` other than the default (`"json"`, `"arraybuffer"`, `"blob"`) were never captured**, silently throwing inside `response-capture.js`'s hook (swallowed by its own try/catch) — and because the request's slot in the response-body matching queue was never filled, the *next* request to the same URL could have its response body wrongly attached to the earlier, unrelated one. `json`-typed responses are now captured (serialized back to text); `arraybuffer`/`blob` still can't be read as text but no longer leave a stuck queue entry behind.
- **Requests made from inside an iframe were never captured**, since the content scripts that hook `fetch`/`XHR` and relay captured bodies only ran in the top-level frame (`chrome.webRequest` itself has no such restriction, so the request still showed up with no body, and — same root cause as above — could steal a later same-URL request's body). Content scripts now run in every frame (`all_frames: true`).
- **The Size column could get stuck blank** for a response with no `Content-Length` header, whose size is only known once the response body arrives separately — the popup only refreshed a row's size/duration cells when its status code changed, which doesn't happen on that second update, so the correct size that had in fact been captured never reached the visible table (the detail panel showed it correctly, since it reads storage directly).
- **HAR export's request time was wrong by 1000x for any request ≥1 second.** It parsed the already-formatted display string (e.g. `"2.5s"`) instead of the raw millisecond value, so `parseFloat("2.5s")` produced `2.5` — a HAR file claiming a 2500ms request took 2.5ms. Now uses the raw duration.
- **A request that failed almost instantly could show a blank Time column** instead of its actual (short) duration, if `onErrorOccurred` read back its start-time before `onBeforeRequest`'s own write had finished — a narrow race made wider by sitting behind the same shared queue used to keep `REQUEST_ORDER_KEY` consistent. The request's own start-time is now written immediately instead of after that queue.
